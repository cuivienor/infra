# Outputs for Proxmox homelab containers
...
output "wishlist_container_id" {
  value       = proxmox_virtual_environment_container.wishlist.vm_id
  description = "Container ID (CTID) for the wishlist container"
}

output "wishlist_container_ip" {
  value       = proxmox_virtual_environment_container.wishlist.initialization[0].ip_config[0].ipv4[0].address
  description = "IP address of the wishlist container (static)"
}
