/**
 * Cuiv-skills plugin for OpenCode.ai
 *
...
export const CuivSkillsPlugin = async ({ client, directory }) => {
  const homeDir = os.homedir();
  const projectSkillsDir = path.join(directory, '.opencode/skills');
  // Derive cuiv-skills skills dir from plugin location (works for both symlinked and local installs)
  const superpowersSkillsDir = path.resolve(__dirname, '../../skills');
  const personalSkillsDir = path.join(homeDir, '.config/opencode/skills');

  // Helper to generate bootstrap content
  const getBootstrapContent = (compact = false) => {
    const usingCuivSkillsPath = skillsCore.resolveSkillPath('using-cuiv-skills', superpowersSkillsDir, personalSkillsDir);
    if (!usingCuivSkillsPath) return null;

    const fullContent = fs.readFileSync(usingCuivSkillsPath.skillFile, 'utf8');
...
**Skills naming (priority order):** project: > personal > cuiv-skills:`
...
- Cuiv-skills skills: \`cuiv-skills:skill-name\`
- Project skills override personal, which override superpowers when names match`;

    return `<EXTREMELY_IMPORTANT>
You have cuiv-skills.

**IMPORTANT: The using-cuiv-skills skill content is included below. It is ALREADY LOADED - you are currently following it. Do NOT use the use_skill tool to load "using-cuiv-skills" - that would be redundant. Use use_skill only for OTHER skills.**
  ...
  return {
    tool: {
      use_skill: tool({
        ...
        args: {
          skill_name: tool.schema.string().describe('Name of the skill to load (e.g., "cuiv-skills:brainstorming", "my-custom-skill", or "project:my-skill")')
        },
        execute: async (args, context) => {
          ...
          if (forceProject || !skill_name.startsWith('cuiv-skills:')) {
...
# ============================================`;
      ...
      find_skills: tool({
        description: 'List all available skills in the project, personal, and cuiv-skills libraries.',
        args: {},
        execute: async (args, context) => {
          ...
          const superpowersSkills = skillsCore.findSkillsInDir(superpowersSkillsDir, 'cuiv-skills', 3);

          // Priority: project > personal > superpowers
          const allSkills = [...projectSkills, ...personalSkills, ...superpowersSkills];

          if (allSkills.length === 0) {
            return 'No skills found. Install cuiv-skills skills to ~/.config/opencode/superpowers/skills/ or add project skills to .opencode/skills/';
          }
